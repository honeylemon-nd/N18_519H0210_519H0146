...
{
    public partial class FormReceiptInputInfo : Form
    {
        //tạo một biến để hứng giá trị usename đang login vào
        //mục đích để có dc id, displayName của user đang đăng nhập
        public static string userName;
        public FormReceiptInputInfo( string UserNameLogining)
        {
            InitializeComponent();
            //để giá trị vào các combobox
            InputConrol.GetListObjectToCMB(cmb_DisplayNameObject);
            InputConrol.GetListDisPlayNameUSerToTextBox(txb_DisplayNameUser,UserNameLogining);
            //lock các button lại
            LockTextBox();
            //giá trị mặt định của ngày lập phiếu = ngày hôm nay
            dtp_DateInput.Value = DateTime.Now;
        ...
        private void Btn_AddOjectToList_Click(object sender, EventArgs e)
        {
            ...
            // nếu sl dòng >0 thì phải kiểm tra
            if (dgv_View.Rows.Count > 0)
            {
                ...
                dgv_View.Rows.Add(cmb_DisplayNameObject.Text, nud_Cout.Value, nud_InputPrice.Value);
                btn_DeleteObjectToList.Enabled = true;
                btn_LapPhieu.Enabled = true;
        ...
        private void Btn_LapPhieu_Click(object sender, EventArgs e)
        {
            ...
            bool IsSuccess=InputConrol.AddInputToData(dgv_View, dtp_DateInput, userName);
